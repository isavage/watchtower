...
export function OpsPage(){
 ...
 return <div className="grid gap-5">
  <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-xl font-semibold text-ink-900">VPS Operations</h1><p className="mt-1 text-sm text-ink-500">Read-only security, access, services, and host health overview.</p></div><a href="/" className="rounded-lg bg-ink-900 px-3 py-2 text-sm font-medium text-white hover:bg-ink-800">Dashboard</a></div>
  ...
  <Card title="Listening ports"><List items={data.listeners.items.map((x:any)=>`${x.address} · pid ${x.pid??"—"}`)} empty={data.listeners.error ?? "No listeners returned"}/></Card>
  <Card title="Services & containers"><h3 className="mb-2 text-xs font-medium text-ink-500">Docker containers</h3><List items={data.services.docker.map((x:any)=>`${x.name} · ${x.status}${x.ports?` · ${x.ports}`:""}`)} empty={data.services.docker_error ?? "No containers returned"}/><h3 className="mt-4 mb-2 text-xs font-medium text-ink-500">Failed systemd units</h3><List items={data.services.failed_systemd} empty={data.services.systemd_error ?? "No failed units reported"}/></Card>
